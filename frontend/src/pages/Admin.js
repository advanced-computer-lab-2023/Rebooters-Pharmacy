...
function Administrator() {
  ...
  const [suggestedPharmacistUsernames, setSuggestedPharmacistUsernames] =
    useState([]);
  ...
  const viewPharmacists = async () => {
    try {
      ...
      if (!data || data.length === 0) {
        setSubmissionviewPharmacists("error");
        setMessage(
          `No pharmacist found with the username "${searchTermPharmacist}".`
        );
        setPharmacists([]);
      ...
      console.log(
        "Processed Data:",
        data.map((pharmacist) => pharmacist.username)
      );

      setSuggestedPharmacistUsernames(
        data.map((pharmacist) => pharmacist.username)
      );
  ...
  const viewPatients = async () => {
    try {
      ...
      if (!data || data.length === 0) {
        setSubmissionStatusViewPatient("error");
        setMessage(
          `No patient found with the username "${searchTermPatient}".`
        );
        setPatients([]);
      ...
      console.log(
        "Processed Data:",
        data.map((patient) => patient.username)
      );
                ...
                >
                  <Card.Body>
                    ...
                    <div
                      ...
                      onClick={() =>
                        setShowPharmacistDropdown(!showPharmacistDropdown)
                      }
                    >
                      ...
                      {suggestedPharmacistUsernames.length > 0 &&
                        showPharmacistDropdown && (
                          <ul className="list-group position-absolute w-50 mt-1">
                            {suggestedPharmacistUsernames.map(
                              (username, index) => (
                                <li
                                  key={index}
                                  className="list-group-item clickable"
                                  onClick={() =>
                                    handlePharmacistDropdownSelection(username)
                                  }
                                >
                                  {username}
                                </li>
                      ...
                      {submissionStatusviewPharmacists === "error" &&
                        !suggestedPharmacistUsernames.length && (
                          <div className="alert alert-danger mt-2">
                            Not a valid pharmacist username.
                          </div>
                        )}
                    </div>
                ...
                >
                  <Card.Body>
                    ...
                    >
                      ...
                      {suggestedPatientUsernames.length > 0 &&
                        showPatientDropdown && (
                          <ul className="list-group position-absolute w-50 mt-1">
                            {suggestedPatientUsernames.map(
                              (username, index) => (
                                <li
                                  key={index}
                                  className="list-group-item clickable"
                                  onClick={() =>
                                    handlePatientDropdownSelection(username)
                                  }
                                >
                                  {username}
                                </li>
                      ...
                      {SubmissionStatusViewPatient === "error" &&
                        !suggestedPatientUsernames.length && (
                          <div className="alert alert-danger mt-2">
                            Not a valid patient username.
                          </div>
                        )}
                    </div>
            ...
            <div className="mt-4">
              <div className="d-flex flex-column align-items-center">
                <Card
                  className="mt-4"
                  border="success"
                  style={{ width: "70%", borderWidth: "2px" }}
                >
                  <Card.Body>
                    <Card.Title
                      className="text-center"
                ...
                {showPharmacistRequests && (
                  <div className="mt-2">
                    {newPharmacistRequestData.length > 0 ? (
                      ...
                      <Card
                        style={{
                          backgroundColor: "rgb(26, 188, 188)",
                          color: "black",
                          width: "18rem",
                        }}
                        className="mb-2"
                      >
                        ...
                        <Card.Body>
                          <Card.Title>
                            No new pharmacist requests available.
                          </Card.Title>
                          <Card.Text>
